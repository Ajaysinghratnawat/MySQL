...
-- Student detail whether they have enroll or not 
-- is use to check null or not
select s.student_id,s.student_name,e.enrollment_id,e.course_name
from students as s left join enrollments as e
on s.student_id=e.student_id where e.enrollment_id is null;

-- student and number of course enrolled
select s.student_id,s.student_name,count(e.course_name)
from students as s join enrollments as e
where s.student_id=e.student_id group by student_id,student_name;
